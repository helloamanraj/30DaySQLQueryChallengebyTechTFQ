...
Solution 1:
```sql
Select category, count(model) as number_of_bikes from bike
group by category
...
Solution 2:
```sql
select  name, count( membership_type_id) as membership_count from customer as c
inner join membership as m on c.id = m.customer_id
group by name
